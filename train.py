...
# @Author   :Zhangjinzhou
# @Time     :2019/4/17 17:58
# @Filename :train.py

import tensorflow as tf
from datetime import datetime
from utils import get_next_batch,CAPTCHA_HEIGHT, CAPTCHA_WIDTH, CAPTCHA_LEN, CAPTCHA_LIST
...
def train(height=CAPTCHA_HEIGHT, width=CAPTCHA_WIDTH, y_size=len(CAPTCHA_LIST)*CAPTCHA_LEN):
    ...
    step = 0    # 步数
    while step<1000 :
        batch_x, batch_y = get_next_batch(60)
        acc = sess.run(optimizer, feed_dict={x: batch_x, y: batch_y, keep_prob: 0.75})
        # 每训练一百次测试一次
        if step % 100 == 0:
